...
export default {
    async fetch(request, env) {
        ...
        const pathToken = pathSegments.length > 0 ? pathSegments[0] : null;
        
        // 检查是否是路径TOKEN访问
        const isPathTokenAccess = pathToken && (pathToken === 临时TOKEN || pathToken === 永久TOKEN);
        
        // 如果通过路径TOKEN访问，重写URL路径（移除TOKEN部分）
        if (isPathTokenAccess) {
            const newPath = '/' + pathSegments.slice(1).join('/');
            url.pathname = newPath || '/';
        }
        //https://ipcheck.xxx.workers.dev/check?proxy=xxx/token=xxx  https://ipcheck.xxx.workers.dev/check?proxy=xxx&token=xxx  
        // 处理检测请求
        if (url.pathname.toLowerCase() === "/check") {
            // 验证TOKEN（支持路径TOKEN和参数TOKEN）
            const hasValidToken = isPathTokenAccess || 
                                 (url.searchParams.has('token') && 
                                  (url.searchParams.get('token') === 临时TOKEN || 
                                   url.searchParams.get('token') === 永久TOKEN));
            
            if (env.TOKEN && !hasValidToken) {
        ...
        if (url.pathname === '/' || url.pathname === '') {
            if (env.TOKEN && !isPathTokenAccess) {
                // 设置了TOKEN但没有通过路径TOKEN访问，显示nginx页面
                ...
                // 如果是路径TOKEN访问，传递TOKEN信息给HTML页面
                return await HTML(网站图标, 网络备案, img, isPathTokenAccess ? pathToken : null);
            }
        }
...
async function HTML(网站图标, 网络备案, img, pathToken = null) {
// 如果通过路径TOKEN访问，自动在API请求中包含TOKEN
    ...
    <script>
        ...
        window.fetch = function(url, options = {}) {
            if (typeof url === 'string' && (url.includes('/check') || url.includes('/ip-info'))) {
                ...
                url = url + separator + 'token=${pathToken}';
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (header) {
                ...
                authNote.style.fontSize = '0.9em';
                authNote.style.opacity = '0.8';
                authNote.style.marginTop = '5px';
                authNote.textContent = '✓ 已通过路径TOKEN授权访问';
...
<svg viewBox="0 0 250 250" aria-hidden="true">
  ...
  <script>
      ...
      async function fetchEntryInfo(host, retryCount = 0) {
          try {
              const tokenParam = '${pathToken || ''}'; // 使用pathToken变量
              const tokenQuery = tokenParam ? '&token=' + encodeURIComponent(tokenParam) : '';
              const response = await fetch('/ip-info?ip=' + encodeURIComponent(host) + tokenQuery);
              const data = await response.json();
        ...
        async function checkProxy() {
            ...
            try {
                ...
                // 检查是否是域名
                if (!isIPAddress(host)) {
                    entryInfo.innerHTML = '<div class="loading"><div class="spinner"></div>Resolving domain...</div>';
                    ...
                    try {
                        ...
                        console.log('Domain ' + host + ' resolved to IP: ' + targetIP);
                        console.log('All IPs: ' + currentDomainInfo.all_ips.join(', '));
                    } catch (dnsError) {
                        entryInfo.innerHTML = '<div class="error">Domain resolution failed: ' + dnsError.message + '</div>';
                        exitInfo.innerHTML = '<div class="error">Domain resolution failed: ' + dnsError.message + '</div>';
                        return;
                    }
                }
                
                // 同时开始获取入口和出口信息
                entryInfo.innerHTML = '<div class="loading"><div class="spinner"></div>Getting entry info...</div>';
                exitInfo.innerHTML = '<div class="loading"><div class="spinner"></div>Checking proxy...</div>';
                ...
                if (exitPromise.status === 'fulfilled') {
                    ...
                    if (!proxyData.success) {
                        ...
                        exitInfo.innerHTML = '<div class="error">Proxy check failed: ' + (proxyData.error || 'Please check proxy link') + '</div>';
                    } else {
                        formatInfoDisplay(proxyData, 'exitInfo', false, proxyData.responseTime);
                    }
                } else {
                    document.getElementById('exitInfoTitle').textContent = 'Exit Info(Proxy Unavailable)';
                    exitInfo.innerHTML = '<div class="error">Proxy check failed, please try again later</div>';
                }
                
            } catch (error) {
                console.error('Check process error:', error);
                entryInfo.innerHTML = '<div class="error">Check failed, please try again later</div>';
                document.getElementById('exitInfoTitle').textContent = 'Exit Info(Proxy Unavailable)';
                exitInfo.innerHTML = '<div class="error">Check failed, please try again later</div>';
            } finally {
                checkBtn.disabled = false;
            }
        }
